...
| 07:30 | Final publish decision | Orchestrator | `08_final_report.md` |
...
2. If the run is `NO_TRADE` or `HALTED`, still publish the output package on schedule.
3. Midday and pre-close reviews do not change the morning thesis unless a stop criterion is triggered.
4. The evolution pass never edits protected rules automatically.
...
1. Run manifest.
2. Regime and preflight note.
3. Top candidates or explicit no-trade logic.
4. Portfolio proposal or no-trade decision.
5. Risk review.
6. Final report.
7. Close log.
8. Evolution log.
