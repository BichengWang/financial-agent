...
- **Recreate:** Claude Code `CronCreate`, cron `27 7 * * 1-5`, durable, with a self-contained prompt that executes `agents/equity/daily_investment_system/main.md` (repo-relative — never hardcode a `/Users/...` machine path; that killed the previous job).
...
0. **Prediction Settlement** — settled table `Ticker | Vintage | Entry | Target Date | mu | Realized Return | SPY Return | Alpha | Direction | CI Result | z`; `MARKET_FORECAST` settlements (core ETFs) sit in the same table with `SPY Return` / `Alpha` = `N/A` and direction scored on raw return per `rules.md § Settlement Rules`; rolling calibration metrics (or `INSUFFICIENT_SETTLED_N`) with raw `n`, 28-day `eff_n`, and Track A calibration-proposal eligibility shown for each record type and the market-forecast line reported separately; which prior `15_predictions.json` files were scanned.
...
### `13_evolution_log.md`
Run context (date, status, regime, evaluation window, ledger status including raw `n` and 28-day `eff_n`, baseline flag); what worked / what failed; primary diagnosis (one of: data quality, regime classification, factor calibration, portfolio construction, risk review, output clarity, source grounding); exactly one proposed change with **Track A/B classification** per `rules.md § Evolution Policy`; hypothesis; validation (Track A: disclose the `n >= 20` and `eff_n >= 3` calibration-evidence gates plus holdout/IR/hit-rate/drawdown/turnover deltas; Track B: the three-condition standard); decision `ACCEPT` / `REJECT` / `DEFER` / `NO_CHANGE_ACCEPTED`; effective next step.
