...
8. **Tie-break (mandatory).** When two or more folders sit at the same `|folder_date - target|`, do not pick arbitrarily — apply in order: (a) same model family as the executing model; (b) has a usable `15_predictions.json` (file present and containing at least one prediction record); (c) lexicographic folder name. Then **report every tied candidate** in `02_reflection.md § 1` with its own settled hit rate, mean alpha, and mean z, and state explicitly whether the MoM conclusion is invariant across them. Tied folders are not interchangeable: observed spreads across tied books reached 48pp of hit rate on 2026-07-29 and 40.7pp on 2026-07-30, driven by book composition (near-disjoint name sets, one semiconductor-weighted and one defensive) rather than by luck. A baseline chosen without disclosing the alternatives silently selects the MoM narrative.
...
- `Tech_Z` construction: equal-weight exactly six distinct inputs — 20-bar momentum, 60-bar momentum, daily/weekly MA alignment, daily/weekly MACD state, volume confirmation, and 60-day drawdown. Keep 20/60-bar relative strength versus SPY as displayed/ledgered diagnostics, not additional score slots, because its cross-sectional z-score duplicates momentum under a common benchmark (Track B effective 2026-08-03).
